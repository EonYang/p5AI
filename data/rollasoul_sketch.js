...
},{"./GeometricBeing":1,"./common/Sprite":15,"./common/p5Instance":25,"./common/settings":26}]},{},[27]);//not so serious coffee ground tarot engine to give you early morning joys :) 
...
function Particle() {  
  ...
  this.display = function(){
    ...
    fill(255,random(1,100));
  	...
  	//ellipse (width/4-i*radius, height/4*radius/20*random(-1,1)+ height/2, radius,radius); 
    //ellipse (width/4*3+i*radius, height/4*radius/20*random(-1,1)+ height/2, radius,radius); 
    fill(255,random(1,40));
    ellipse (cos(noise(t)*3*PI)*width/5*1.3+width/2,sin(noise(t)*3*PI)*height/5*1.3+height/2,radius/2,radius);
  		//e1=ellipse (-width/4+i*radius, height/4*radius/20*random(-1,1)+ height/2, radius,radius);
  t++;  
  };
}
...
function draw() {
  ...
  //t+=0.003;
  //count+= 0.1;
  //range = noise(0.1,0.1)*count*count*0.8;
  range += 0.001;
  ...
  // x = cos(angle)*range*sin(angle) + centerX*tan(angle);
   //y = sin(cos(angle)*angle*yfreq)*range*yscl*cos(angle) + centerY;
    ...
    // mouseY controls yscl (verticality)
		// i = 0;
		// i++;
		// fill(random(0, 255), 255, random(20, 30));
		// noStroke();
		// if(px) ellipse(width, 20, 20)
...
}var portName = '/dev/ttyUSB1';  // fill in your serial port name here
...
function setup() {
  ...
  // initiate particle system for water
  system = new ParticleSystem(createVector(width/2 - 135, 404));
  
  // set tree growth direction rate
  frameRate(120);
  e = random (-1, 1)
  e = e * random(30, 50)
  
  // create starting points for bonsai leaf growth, depending on tree branches ending
  for ( var i = 0; i < num; i++ ) {
...
function draw() {
  ...
  ellipse(width / 2, height / 2 + 100, 300, - 35);
   ...
   //draw water level in pot, regulate via drying time
   if (water_up > 1){
   waterLevel();
   }
   dry = dry + 1
   
   // let water evaporate slowly
   if (dry == 400){
    water_up = water_up - 1
    dry = 0;
   }
  ...
  // clicked anywhere after 2 seconds
  if(rain == 1) {
      mousePressed();
      if (grow == 1){ 
  ...
  //draw leaves, grow bonsai tree faster if it got water via growth-rate
  ...
  //regulate growth and cell cycle via shift of elements
  ...
  if (cell_cycle == 10) {
    ...
    // Constrain all points above pot
  ...
  // Draw a separate line connecting other points
...
// Create water drops based on particle class from p5 examples
...
Particle.prototype.display = function() {
  ...
  //fill(random(180, 200), random(220,230), 255, this.lifespan);
...
function waterPlant(){
  ...
  // draw particles around water stream
  stroke(230, 30, 230)
  r = random (-1, 1);
  if (r < 0) {
  system.addParticle();
...
function waterLevel() {
  if (water_up < 8) {
    //draw increasing water level in pot (while plant gets watered)
     strokeWeight(0.5)
     //fill(255);
     fill (180, 243, 250, 95)
     stroke(255, 99);
     ...
     //draw maximum water level in pot
     water_up = 8;
     strokeWeight(0.5)
     //fill(255);
     fill (180, 243, 250, 95)
...
function draw() {
  if (snapshot) {
	...
	// image(myImage, 0, 0);
	// //get average brightness of image and match it to card rank in tarot set;
	// getImageLightness(snapshot,function(brightness){
	// 	console.log("rank: " + rank); //can somehow not access "rank" as a global variable ...???
	// 	// search as well for king, queen, knight and page ranks
	// 	if (rank > 21){
	// 		extra_ranks = ['page', 'knight', 'queen', 'knight'];
...
function setup() {
  ...
  print(pix);
  // let rand = random(100, (myImage.pixels.length));
  // print(rand);
  // for (i = 100; i < 1000; i++){
  //   print(myImage.pixels[i]);
  // 	}
  //print(myImage.pixels());
  
  
  image(myImage, 0, 0);
  //getAverageRGB(myImage);
...
}var portName = '/dev/cu.usbmodemFD111';  // fill in your serial port name here
var options = { baudrate: 115200}; // change the data rate to whatever you wish
...
function preload(){
 data = loadJSON('https://api.nasa.gov/planetary/apod?api_key=c75f19Ii5wvzvtfabokYqZqBOSAFG9rDXbXtCFSB');
}
...
class Fish {
 ...
 data = loadJSON('https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?sol=1000&api_key=c75f19Ii5wvzvtfabokYqZqBOSAFG9rDXbXtCFSB
');
}
function setup() { 
...
function draw() { 
 ...
 data = loadJSON('https://api.nasa.gov/EPIC/api/natural/images?api_key=DEMO_KEY ');
}
function setup() { 
  createCanvas(400, 400);
  console.log(data);
...
function draw() { 
  ...
  //create slider tie to mouseY, put it in another canvas
...
function draw() {
  ...
  //create triangular shapes pattern
  if (x == width/4 || y == height/4){
  console.log("yo");
  z = random(10);
  t = random(1);
...
function draw() {
  counter ++;
  //create slider and tie it to mouseY
  if (counter > 10000){
    background(0,10);
  }
  push();
...
function draw() { 
  ...
  noFill();
  //stroke(255)
  //line(width/2 - 7.5,height/2,140, 40);
  //triangle(width/2 + 7.5,y1,width/2 + 15,y1, 255, height/2);
  //pop();
  if (random(1) > 0.5){
    //strokeWeight(1)
  	// x += 1
...
function draw() {
  ...
  for (var i = 0; i < mouseY*20/100; i++){
    ...
    //arc(0, mouseX + i*20, i*50, i*50, PI+QUARTER_PI, TWO_PI);
    noFill();
    stroke(0);
...
//set number of brownian motion elements (might break the memory of your machine ...)) 
var num = 800;
var range = 100;
...
function draw() {
  background(230, 50);
  //draw background noise as stars
  strokeWeight(10);
  stroke(255, random(0, 10));
  fill(random(250), 230, 180, random(0, 40))
  ellipse(mouseX, mouseY, random(20, 100), random(20, 100))
  //draw color circles around mouse endpoints
  strokeWeight(10);
  stroke(255, random(0, 10));
  fill(random(280), 230, 180, random(0, 40))
  ellipse(mouseX + 10, mouseY + 10, random(20, 100), random(20, 100))
  
  // draw overlaying white geometric structure
  // push mouse positions in array
  var x1 = mouseX + 10;
  var y1 = mouseY + 10;
  rand_val.push(x1);
  rand_val.push(y1);
  f = rand_val;
  // draw array as points and connect them with lines and curves
  // add flickering via random opacity and slight deviations from positions
  // add three-dimensionality with darker lines as shades
  ...
  //create color shapes, curves and black hole in the background based on brownian motion
  for (var j = 1; j < num; j++) {
    ...
    //bezier(300 + j / 20, 200, 40 / j + j / 20, 30 / j + j / 20, random(100), random(800, j), 400 + j / 20, 400 + j / 20, 800 + j / 20, 300 + j / 20)
  }
  //draw text on top
...
function draw() {
  //draw header in white space on right side
  fill(120);
  ...
  // draw folded structure top-left
  fill(253);
...
function windowResized() {
  ...
  // draw illustration line with curved edges and end circles
  noFill();
  stroke(230);
  ...
  // draw folded structure in the middle
  stroke(220, 80);
  fill(253);
  ...
  // overlay parts of poodle base with triangle of structure
  fill(250)
  triangle(550, 19924, 220, 19860, 489, 19630);
  
  //draw bottom waterfall and pond
  noStroke();
  fill(146, 252, 242, 98)
  ...
  //draw illustration line 3 (bottom of page) with end circles
  stroke(230);
  noFill(0);
  ...
  //draw annotation 3 (bottom of page)
  fill(255);
  ...
  // draw illustration line with end circles
  stroke(230)
  ...
  //draw glass
  // stroke(255, 86)
  // ellipse(340, 19680, 200, 20)
  // noFill();
  // arc(345, 19880, 160, 20, 0, PI, PI)
  // stroke(255, 96)
  // line(240, 19680, 265, 19880)
  // line(440, 19680, 425, 19880)
  ...
  // draw mirrored folded structure below 
  fill(253)
  triangle(550, 520, 820, 460, 710, 700);
  //rect(150, 120, 100, 80)
  fill(250)
