...
function preload() {
  data = loadJSON('https://blockchain.info/ticker');
}
...
function draw() { 
  ...
  beginShape();
 vertex(253, 77); // not curve

  curveVertex(253, 77); // control
  curveVertex(221, 68);
  curveVertex(175, 61);
  //curveVertex(133, 69); // control

  vertex(133, 69); // not curve
  vertex(148, 101);// not curve


  curveVertex(148, 101);// control
  curveVertex(183, 92);
  curveVertex(223, 87);
  curveVertex(253, 85); // control

  vertex(253, 85); // not curve
  endShape(CLOSE);
}/*	things to try before Thursday's office hour:
    ...
    2.	draw columns vertically first. the middle column should be sin(PI/2)
		3.	use noise to make each column a little bit curly */
...
/* I think P5.js web editor's loading JSON file is broken*/
...
function setup() { 
  createCanvas(400, 400);
	// create the pumpkin's 2D array for vertices on the right side
    for(let i = 0; i < coord_Left.length; i++) {
        coord_Right[i] = [];
        for(let j = 0; j < coord_Left[i].length; j++) {
            let x_Left = coord_Left[i][j][0];
...
function setup() {
    ...
    // create the background's 2D grid's first dimension
    grid = new Array(23);
    // create the background's 2D grid's second dimension
...
function draw() {
    // define the background's color and stroke
    background(0);
    stroke(239, 193, 6);
    strokeWeight(1);
    noFill();
...
function draw_PumpSects() {
    ...
    for (let i = coord_Left.length - 1; i >= 0 ; i--) {
        ...
        // the first point of the curve - the top point
				if (i == coord_Left.length - 1) {
					y_Actual = y_Original;
        ...
        // the last point of the curve - the top point
				if (i == coord_Left.length - 1) {
					y_Actual = y_Original;
					curveVertex(x_Center, y_Actual);
				} else {
...
// variables for vertex0 @ left front section of the pumpkin
...
// variables for vertex1 @ left front section of the pumpkin
...
// variables for vertex2 @ left front section of the pumpkin
...
// variables for vertex3 @ left front section of the pumpkin
...
// variables for vertex4 @ left front section of the pumpkin
...
// variables for vertex5 @ left front section of the pumpkin
...
// // variables for vertex6 @ left front section of the pumpkin
...
// variables for vertex0 @ right front section of the pumpkin
...
// variables for vertex1 @ right front section of the pumpkin
...
// variables for vertex2 @ right front section of the pumpkin
...
// variables for vertex3 @ right front section of the pumpkin
...
// variables for vertex4 @ right front section of the pumpkin
...
// variables for vertex5 @ right front section of the pumpkin
...
// // variables for vertex6 @ right front section of the pumpkin
...
function shape_Front() {
  ...
  curveVertex(x4_ControlA_LeftFront, y4_ControlA_Front, x4_ControlB_LeftFront, y4_ControlB_Front, x4_Anchor_LeftFront, y4_Anchor_Front);
	...
	// curveVertex(x3_ControlA_RightFront, y3_ControlA_Front, x3_ControlB_RightFront, y3_ControlB_Front, x3_Anchor_RightFront, y3_Anchor_Front);
	// curveVertex(x4_ControlA_RightFront, y4_ControlA_Front, x4_ControlB_RightFront, y4_ControlB_Front, x4_Anchor_RightFront, y4_Anchor_Front);
	// curveVertex(x5_ControlA_RightFront, y5_ControlA_Front, x5_ControlB_RightFront, y5_ControlB_Front, x5_Anchor_RightFront, y5_Anchor_Front);
	// bezierVertex(x6_ControlB_RightFront, y6_ControlB_Front, x6_ControlA_RightFront, y6_ControlA_Front, x6_Anchor_RightFront, y6_Anchor_Front);
...
function shape_Front() {
  ...
  bezierVertex(x4_ControlA_LeftFront, y4_ControlA_Front, x4_ControlB_LeftFront, y4_ControlB_Front, x4_Anchor_LeftFront, y4_Anchor_Front);
	...
	// bezierVertex(x6_ControlB_RightFront, y6_ControlB_Front, x6_ControlA_RightFront, y6_ControlA_Front, x6_Anchor_RightFront, y6_Anchor_Front);
	// // bezierVertex(x5_ControlB_RightFront, y5_ControlB_Front, x5_ControlA_RightFront, y5_ControlA_Front, x5_Anchor_RightFront, y5_Anchor_Front);
	// bezierVertex(x4_ControlB_RightFront, y4_ControlB_Front, x4_ControlA_RightFront, y4_ControlA_Front, x4_Anchor_RightFront, y4_Anchor_Front);
	// bezierVertex(x3_ControlB_RightFront, y3_ControlB_Front, x3_ControlA_RightFront, y3_ControlA_Front, x3_Anchor_RightFront, y3_Anchor_Front);
	// bezierVertex(x2_ControlA_RightFront, y2_ControlA_Front, x2_ControlB_RightFront, y2_ControlB_Front, x2_Anchor_RightFront, y2_Anchor_Front);
	// bezierVertex(x1_ControlA_RightFront, y1_ControlA_Front, x1_ControlB_RightFront, y1_ControlB_Front, x1_Anchor_RightFront, y1_Anchor_Front);
	// bezierVertex(x0_ControlA_RightFront, y0_ControlA_Front, x0_ControlB_RightFront, y0_ControlB_Front, x0_Anchor_RightFront, y0_Anchor_Front);
	// vertex(x_Vertex_Front, y_Vertex_Front);
...
// animation of shatteringlet width = 510;

// variables for the rindCenterLVertexes
// x variables
let xCenterLVertex0 = 256;
let xCenterLBezierA0 = 240, xCenterLBezierA1 = 231, xCenterLBezierA2 = 222;
let xCenterLBezierB0 = 200, xCenterLBezierB1 = 203, xCenterLBezierB2 = 201;
...
//create array of rinds and stem
// rindsLeft
...
function dots() {
	...
	// for (let i = 1; i < arrayRindCenterL.length; i++) {
	for (let i = 1; i < 2; i++) {
	...
	// for (let i = 0; i < arrayRindCenterL.length; i++) {
	// for (let m = 0; m < (arrayRindCenterL[i].y - arrayRindCenterL[0].y); m++) {
	// for (let n = 0; n < (arrayRindCenterL[i].x - arrayRindCenterL[0].x); n++) {
	// print("m: " + m + "n: " + n);
	// fill(0);
	// noStroke();
	// ellipse(n * 15, m * 15, 14, 14);
	// }
	// }
	// }
}
...
function draw() {
    background(0);
    stroke(239, 193, 6); // ? how to use a variable to represent the color
    strokeWeight(2);
    noFill();

    //draw triangles using all the points created above
    ...
    //draw the left half of the pumpkin
    rindMostLeft(0, 0);
    rindMoreLeft(0, 0);
    rindLeft(0, 0);
    rindCenterLeft(0, 0);
...
function rindLeft() {
    ...
    bezierVertex(xLBezierA0, 101, xLBezierA1, 110, xLBezierA2, 119); // hold option key to move cursor a word forward or backward
...
function rindRight() {
    ...
    bezierVertex(width - xLBezierA0, 101, width - xLBezierA1, 110, width - xLBezierA2, 119); // hold option key to move cursor a word forward or backward
...
function stem(){
    ...
    Questions:
        ...
        https://alpha.editor.p5js.org/yy909/sketches/Sy1tJvZh-
        However, after I use functions, I couldn't call them to a correct position
        
    2. How to add texture using the dot maps created?
    		**p5.js reference https://p5js.org/examples/3d-textures.html
        
    		I created two dot maps for the rinds and the stem. 
        rindDots: https://alpha.editor.p5js.org/yy909/sketches/SknnkwZnW
        stemDots: https://alpha.editor.p5js.org/
      	But how can I put them in the shape using texture()? I don't know what call
       	inside the ().
    
    3. ShearZ?
    		**p5.js reference https://p5js.org/reference/#/p5/shearY
    ...
    4. How to only apply the shattering effect to the pumpkin layer?
    		**The shattering effect https://codepen.io/zadvorsky/pen/dILAG
    
    5. How to mirror the left half to the right?
    
    6. How to make the joints not sharp?
    		**p5.js reference https://p5js.org/reference/#/p5/bezierVertex
        Why cannot I use smooth()? https://p5js.org/reference/#/p5/smooth
 */
...
function rindLeft(x, y) {
  ...
  beginShape();
  vertex(253, 104);
  bezierVertex(231, 101, 210, 110, 189, 119); // hold option key to move cursor a word forward or backward
...
function rindCenter(x, y) {
  ...
  beginShape();
  vertex(253, 116);
  bezierVertex(240, 113, 231, 118, 222, 123); // ? (231, 118) is the anchor point. It supposed to be put in the end, but it will look weird
  fill("red");
  stroke("white");
...
// function texture(){
//     let pg = createGraphics(70, 265);
//     pg.rindCenter(-205, -215);
//     texture(pg);
//     shearZ(PI/4.0);
//
// }
...
// animation of all of them

// how to make the line a little bit curlyfunction setup() { 
  createCanvas(400, 400);
} 
...
function draw() {
  ...
  noFill();
  strokeWeight(4);
  // draw all the points below using beginShape(), endShape()
...
}//It's so weird that everytime you have to "play" the code twice so that the background will show up...
...
function setup() {
    ...
    background = createImg("https://image.ibb.co/gcZKtk/fb_before.png");
  	background.hide();
    image(background, 0, 0, background.elt.width, background.elt.height);
    // gif = createImg("https://media.giphy.com/media/3o7aD9NeKlFseQcuRi/giphy.gif");
    // gif.hide();
    tab0 = createImg("https://image.ibb.co/kJrGOk/1.png");
    tab0.hide();
    tab1 = createImg("https://image.ibb.co/iM76Ok/2.png");
    tab1.hide();
    tab2 = createImg("https://image.ibb.co/mF2HUQ/3.png");
    tab2.hide();
    tab3 = createImg("https://image.ibb.co/m5ACUQ/4.png");
    tab3.hide();
    clickMe = createImg("http://cdn.mysitemyway.com/etc-mysitemyway/icons/legacy-previews/icons-256/3d-transparent-glass-icons-arrows/006800-3d-transparent-glass-icon-arrows-solid-hand-points-down.png");
    clickMe.hide();
    rotate(PI/3.0);
...
function setup() {
    ...
    // gif = createImg("https://media.giphy.com/media/3o7aD9NeKlFseQcuRi/giphy.gif");
    // gif.hide();
    img0 = createImg("https://image.ibb.co/kJrGOk/1.png");
    img0.hide();
    img1 = createImg("https://image.ibb.co/iM76Ok/2.png");
    img1.hide();
    img2 = createImg("https://image.ibb.co/mF2HUQ/3.png");
    img2.hide();
    img3 = createImg("https://image.ibb.co/m5ACUQ/4.png");
    img3.hide();
    noStroke();
    r = random(255);
    g = random (255);
...
function mousePressed() {
    ...
    if (d < 95) {
        ...
        text("Happy Birthday!!!", 370, 305);
        image(img0, 296, 320);
			  // setTimeout(image(img1, 296, 320), 3000);
        // let imgs = [img1, img2, img3];
        // for (let i = 0; i <= imgs.length; i++){
        //     setTimeout(image(imgs[i], 296, 320), 3000);
        // }
        // setTimeout(gif, 3000);
    }
}var y = 100;
...
function draw(){
    ...
    // the h is supposed to be 106, but the triangle looks super small when it's 106, why?
...
function drawTopTriangle(xTopLeft, yTopLeft, xBottomLeft, yBottomLeft, xTopRight, yTopRight, darkGrey, lightGrey, axis){
    //no need to claim x2, y2, but is it very organized and easy to read?
    noFill();
